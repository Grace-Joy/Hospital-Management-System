...
void showAllDoctors(vector <doctor> doctorType)
{
    ...
    for (int i=0; i<doctorType.size(); i++)
    {
        ...
        cout<<"The Doctor's speciality: "<<doctorType[i].getDocSpeciality()<<endl;
        cout<<"***************************************"<<endl;
    }
...
void updateDoctorsInfo(vector <doctor> &doctorType)
{
    ...
    if(userChoice==2) //if the user chooses 2, then password is updated
    {
        string oldPassword;
        string newPassword;
start:
        ...
        if(oldPassword==doctorType[indexOfDocToBeUpDated].doc_password) //checks if doctor password exists
        {
            system("CLS");
    ...
    if(userChoice==4) //if the user choice is 4 then the doctor's speciality's is updated
    {
        string newSpeciality;

        cout<<"Enter the doctor's new speciality: ";
        cin >> newSpeciality;
        system("CLS");
        doctorType[indexOfDocToBeUpDated].doc_speciality=newSpeciality; //updates the doctor speciality
        cout<<"The doctor's speciality has been successfully updated!"<<endl;
        system("PAUSE");
        system("CLS");
    }
